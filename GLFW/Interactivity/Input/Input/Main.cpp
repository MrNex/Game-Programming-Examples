...
// True is the mouse button is down, false if it is not.
bool mouseLeftDown;
bool mouseRightDown;

// Previous mouse coordinates, used to determine how far the mouse moved.
double prevMouseX;
double prevMouseY;
...
void update()
{
	...
	rot = glm::rotate(rot, glm::radians(1.0f), glm::vec3(1.0f, 1.0f, 0.0f));
...
void init()
{
	...
	view = glm::lookAt(	glm::vec3(0.0f, 0.0f, 8.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
...
{
	// The key parameter refers to a code that is specific to the key that was pressed. For example: GLFW_KEY_ESCAPE or GLFW_KEY_B
	// The scancode is unique for every key, but is also platform-specific. Different platforms will have different scancodes for the same keys.
	// The action parameter is either GLFW_PRESS, GLFW_RELEASE, or GLFW_REPEAT. Repeat is if the key is being held down.
	// The mods parameter is used to determine if the SHIFT, CONTROL, ALT, or SUPER (such as the Windows key) keys are held down.
	// These are bit parameters, so if multiple are held down, you have to add them together (IE. SHIFT + CONTROL are held down so mods == GLFW_MOD_SHIFT + GLFW_MOD_CONTROL)

	// We'll set it up so that arrow keys move the object around.
	if (action == GLFW_PRESS || action == GLFW_REPEAT) // We want it to continue to move if we hold down the key, so we include repeat too.
	{
		switch (key)
		{
		case GLFW_KEY_UP:
			// Translate in the positive Y direction since the up arrow key is pressed.
			trans = glm::translate(trans, glm::vec3(0.0f, 0.1f, 0.0f));
			break;
		case GLFW_KEY_RIGHT:
			// Translate in the positive X direction since the right arrow key is pressed.
			trans = glm::translate(trans, glm::vec3(0.1f, 0.0f, 0.0f));
			break;
		case GLFW_KEY_LEFT:
			// Translate in the negative X direction since the left arrow key is pressed.
			trans = glm::translate(trans, glm::vec3(-0.1f, 0.0f, 0.0f));
			break;
		case GLFW_KEY_DOWN:
			// Translate in the negative Y direction since the down arrow key is pressed.
			trans = glm::translate(trans, glm::vec3(0.0f, -0.1f, 0.0f));
			break;
		default:
			// Key is not one of the arrow keys, so let's ignore it.
			break;
		}
	}
}

// Fires every time the mouse is moved.
void OnMouseMove(GLFWwindow* window, double xpos, double ypos)
{
	// This gives us an x and y position in window coordinates.
	
	// If the left mouse button is down.
	if (mouseLeftDown)
	{
		// Then let's move the object along the Z axis.
		double diffY = ypos - prevMouseY;

		// Now we have the difference, or the distance the mouse moved. Let's modify our transformation matrix.
		trans = glm::translate(trans, glm::vec3(0.0f, 0.0f, diffY * 0.025f));
	}
	if (mouseRightDown)
	{
		// Then let's move the object along the X axis.
		double diffX = xpos - prevMouseX;

		// Now we have the difference, or the distance the mouse moved. Let's modify our transformation matrix.
		trans = glm::translate(trans, glm::vec3(diffX * 0.025f, 0.0f, 0.0f));
	}

	// Set our previous variables for the next time.
	prevMouseX = xpos;
	prevMouseY = ypos;
}
...
{
	// The button parameter is a code specific to which mouse button was pressed. For example: GLFW_MOUSE_BUTTON_RIGHT
	// The action parameter is either GLFW_PRESS or GLFW_RELEASE.
	// The mods parameter is used to determine if the SHIFT, CONTROL, ALT, or SUPER keys are held down. Not relevant for mouse input usually.
	...
	if (button == GLFW_MOUSE_BUTTON_LEFT)
	{
		...
		{
			// The left mouse button is down, so let's set our boolean.
			mouseLeftDown = true;
		}
		else // action == GLFW_RELEASE
		{
			// The left mouse button was released, so:
			mouseLeftDown = false;
		}
	}
	else if (button == GLFW_MOUSE_BUTTON_RIGHT) // If this is the right mouse button
	{
		if (action == GLFW_PRESS)
		{
			// The right mouse button is down, so let's set our boolean.
			mouseRightDown = true;
		}
		else // action == GLFW_RELEASE
		{
			// The right mouse button was released, so:
			mouseRightDown = false;
		}
	}
}
...
int main(int argc, char **argv)
{
	...
	GLFWwindow* window = glfwCreateWindow(800, 600, "Input", nullptr, nullptr);
	...
	// Sets the callback functions that will trigger every time input is received.
