/*
Title: Calculating the Moment Of Intertia Tensor (Rectangular Prism)
File Name: main.cpp
Copyright © 2015
...
Description:
This is a demonstration calculating & transforming moments of inertia of a rectangular prism in 3D.
...
glm::mat3 inertiaTensor;	//The inertia tensor of the rectangle in model space
...
///
//Calculates the inertia tensor in modelspace (at origin) of a rectangular prism
//
//Parameters:
//	r: The rectangular prism to calculate the moment of inertia of
//	m: the mass of the prism
//
//Returns:
//	The moment of inertia tensor in modelspace for rotation about the origin of the prism
glm::mat3 CalculateInertiaTensorOfBox(float width, float height, float depth, float m)
{
	glm::mat3 inertia;
	inertia[0][0] = m * (powf(depth, 2.0f) + powf(height, 2.0f)) / 12.0f;
	inertia[1][1] = m * (powf(width, 2.0f) + powf(depth, 2.0f)) / 12.0f;
...
void main()
{
	...
	planeVerts = (Vertex*)arr;

	rect = new struct Mesh(36, planeVerts, GL_TRIANGLES);

	//Scale the rectangular prism
	glm::vec3 scale(0.3f, 0.1f, 0.6f);
	rect->scale = glm::scale(glm::mat4(1.0f), scale);

	mass = 10.0f;
	inertiaTensor = CalculateInertiaTensorOfBox(2.0f * scale.x, 2.0f * scale.y, 2.0f * scale.z, mass);
