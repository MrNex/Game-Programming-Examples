...
struct RigidBody
{
	...
	//	coeffOfRolling: The coefficient of rolling resistance
	RigidBody::RigidBody(glm::vec3 pos, glm::vec3 vel, glm::vec3 acc, 
		glm::mat3 rot, glm::vec3 aVel, glm::vec3 aAcc,  
		float mass, 
		float coeffOfRestitution, float dynamicC, float staticC, float coeffOfRolling)
	{
		...
		rollingResistance = coeffOfRolling;
	}
};
...
void ApplyLinearFriction(RigidBody &body1, RigidBody &body2, const glm::vec2 &MTV, const glm::vec2 &collisionPoint)
{
	...
	if(fabs(relVelocityTangential) < FLT_EPSILON)
	{
	...
	{ 
		...
		if(body1.inverseMass != 0.0f && body1.inverseMomentOfInertia != 0.0f)
		{
			...
			glm::vec3 rollingResistance = glm::vec3(0.0f);
			if(glm::length(body1.angularVelocity) > FLT_EPSILON)
			{
				//From this angular impulse we must subtract the torque due to rolling resistance. This will be opposite the direction of the angular frictional impulse.
				rollingResistance = -glm::normalize(body1.angularVelocity);
			}
			else
			{
				rollingResistance = -glm::normalize(angularFrictionalImpulse);
			}
			//The magnitude of this rolling resistance force is equal to:
			//	Frr = Crr * m * g
			//where Frr is the force due to rolling resistance, m is the mass, and g is the force due to gravity
			rollingResistance *= body1.rollingResistance * glm::length(gravity) / body1.inverseMass;
			//Now that we have the force due to rolling resistance we can calculate the applied torque from that, to slow the resulting rotation of the wheel.
			//Trr = Frr * R
			//
			//Where R is the radius to the point of contact.
			rollingResistance *= glm::length(radius1);
			
			//Apply the angular impulse
			if(glm::dot(angularFrictionalImpulse, rollingResistance) > FLT_EPSILON && glm::length(angularFrictionalImpulse + rollingResistance) > glm::length(body1.angularVelocity / body1.inverseMomentOfInertia))
			{
				//If the angular forces would cause the object to switch directions, zero out the velocity
				body1.netAngularImpulse -= (body1.angularVelocity / body1.inverseMomentOfInertia).z;
			}
			else
			{
				body1.netAngularImpulse += angularFrictionalImpulse.z + rollingResistance.z;
			}
	...
	if(glm::length(frictionalImpulse))
	{
		...
		if(body2.inverseMass != 0.0f && body2.inverseMomentOfInertia != 0.0f)
		{
			...
			//From this angular impulse we must subtract the torque due to rolling resistance. This will be opposite the direction of the angular frictional impulse.
			glm::vec3 rollingResistance = -glm::normalize(angularFrictionalImpulse);
			//The magnitude of this rolling resistance force is equal to:
			//	Frr = Crr * m * g
			//where Frr is the force due to rolling resistance, m is the mass, and g is the force due to gravity
			rollingResistance *= body2.rollingResistance * glm::length(gravity) / body2.inverseMass;
			//Now that we have the force due to rolling resistance we can calculate the applied torque from that, to slow the resulting rotation of the wheel.
			//Trr = Frr * R
			//
			//Where R is the radius to the point of contact.
			rollingResistance *= glm::length(radius2);

			//Apply the angular impulse
			if(glm::dot(angularFrictionalImpulse, rollingResistance) > FLT_EPSILON && glm::length(angularFrictionalImpulse + rollingResistance) > glm::length(body2.angularVelocity / body2.inverseMomentOfInertia))
			{
				//If the angular forces would cause the object to switch directions, zero out the velocity
				body2.netAngularImpulse -= (body2.angularVelocity / body2.inverseMomentOfInertia).z;
			}
			else
			{
				body2.netAngularImpulse += angularFrictionalImpulse.z + rollingResistance.z;
			}
		}
	}
...
{	
	...
	circleBody->netForce += (1.0f / circleBody->inverseMass) * gravity;
...
void main()
{
	...
	// Create a window
	window = glfwCreateWindow(800, 800, "Rolling Resistance (2D)", nullptr, nullptr);
	...
	circleBody = new RigidBody(
		...
		0.01f								//Coefficient of rolling resistance
		);
	...
	groundBody = new RigidBody(
		...
		1.0f,								//Static coefficient of friction
		1.0f								//Coefficient of rolling resistance
		);
