...
Description:
This is a demonstration of determining the Minimum Translation Vector of Two convex polygons in 2D.
The minimum translation vector is the direction in which the least amount of motion can be taken
to have the objects no longer intersecting.

The algorithm uses a slightly modified version of the Separating Axis Theorem for 2D convex polygons.
When a collision is detected we check the magnitude of the overlap on that axis.
The axis which has the minimum overlap is the minimum translation vector.
...
void update()
{
	...
	if ((collision = TestIntersection(convexHull1, poly1.center, convexHull2, poly2.center)))
	{
		//Red color
		lineColor.x = 1.0f;
		lineColor.y = 0.0f;
...
void renderScene()
{
	...
	if(collision)
	{
		glm::vec2 endPoint = poly1.center + overlap * minimumTranslationVector;

		glColor3f(0.0f, 1.0f, 1.0f);

		glBegin(GL_POINTS);
		glVertex3f(poly1.center.x, poly1.center.y, 0.0f);
		glEnd();
		...
		endPoint = poly2.center - overlap * minimumTranslationVector;
		glColor3f(1.0f, 1.0f, 0.0f);

		glBegin(GL_POINTS);
		glVertex3f(poly2.center.x, poly2.center.y, 0.0f);
		glEnd();
