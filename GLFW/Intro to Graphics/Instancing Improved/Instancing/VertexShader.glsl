...
uniform mat4 MVP[2]; // Our uniform MVP matrix to modify our position values

void main(void)
{
	color = in_color;	// Pass the color through
	gl_Position = MVP[gl_InstanceID] * vec4(in_position, 1.0); //w is 1.0, also notice cast to a vec4
}